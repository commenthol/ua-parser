...
['firefox_user_agent_strings.yaml', 'test_user_agent_parser.yaml', 'pgts_browser_list.yaml'].forEach(function(fileName) {
	...
	describe(fileName, function() {
		fixtures.forEach(function(f) {
			if (f.js_ua) return;
			it(f.user_agent_string, function() {
				var ua = uaParser.parse(f.user_agent_string).ua;
				fixFixture(f, ['major', 'minor', 'patch', 'type']);
				...
				assert.strictEqual(ua.type, f.type, msg('ua.type', ua.type, f.type));
			});
		});
	});
});
	...
	describe(fileName, function() {
		fixtures.forEach(function(f) {
			it(f.user_agent_string, function() {
				...
				if (f.patch_minor !== null && os.patchMinor !== undefined) {
